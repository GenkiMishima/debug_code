...
subroutine calc_next_step_imp
   ...
   do j=1,nj-1
      do i=1,ni-1
         ...
         RHS(:,i,j)=temp0*dt(i,j)/area(i,j)*( dsj(i  ,j  )*(X_Numerical(:,i  ,j  )-vis_i(:,i  ,j  ))&
   ...
   do j=1,nj-1
      do i=1,ni-1
         temp0=1.d0/alpha(i,j)
         q_plime(:,i,j)=RHS(:,i,j)&
                       +temp0*dt(i,j)/dsi(i,j)*matmul(Ap(:,:,i-1,j),q_plime(:,i-1,j))&
                       +temp0*dt(i,j)/dsj(i,j)*matmul(Bp(:,:,i,j-1),q_plime(:,i,j-1))
      end do
   end do
   !$omp end parallel do
   q_imp(:,ni,:)=0.d0
   ...
   do j=1,nj-1
      ...
      do i=1,ni-1
         ...
         q_imp(:,m,n)=q_plime(:,m,n)&
                     -temp0*dt(i,j)/dsi(m,n)*matmul(Ap(:,:,m+1,n),q_imp(:,m+1,n))&
                     -temp0*dt(i,j)/dsj(m,n)*matmul(Bp(:,:,m,n+1),q_imp(:,m,n+1))
      end do
   end do
   !$omp end parallel do
