...
implicit none
   ...
   double precision, intent(in) , dimension(5) ::wl,wr
   double precision, intent(out), dimension(5) ::flux


   double precision ul,vl,rhol,gmmal,unl,nYl,phil(5),al,Ml,betal,pl
   double precision ur,vr,rhor,gmmar,unr,nYr,phir(5),ar,Mr,betar,pr
  ...
  !set p_tilde
   ...
   ! '(es15.7)',p_tilde
   !calculate tg
   flux=0.5d0*(dm+abs(dm))*phil+0.5d0*(dm-abs(dm))*phir+p_tilde*n
!    '("flux")'
!    '(4es15.7)',flux
...
!double precision, dimension(4)  :: dq, convective_vector, temp, temp_arrey1, temp_arrey2
