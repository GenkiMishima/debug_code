...
use variable
implicit none
double precision, dimension(5) :: Ev, Fv
double precision, dimension(5) :: dwdxi,dwdata,dwdx,dwdr
...
double precision chi,dchidxi,dchideta,hidchidx,hidchidr,dchidx,dchidr
...
do j=1,nj-1
   do i=1,ni
      ...
      dchideta= (w(5,i-1,j+1) +w(5,i,j+1)&
      ...
      hidchidx=0d0
      hidchidr=0d0
      dchidx=dchidxi*geojaci(1,1,i,j)+dchideta*geojaci(2,1,i,j)
      hidchidx=hidchidx+dchidx*chi
      dchidr=dchidxi*geojaci(1,2,i,j)+dchideta*geojaci(2,2,i,j)
      hidchidr=hidchidr+dchidr*chi
      ...
      tau_xr= mu*(dvdx+dudr)
      qx  = -k_heat*dTdx-rho*D*hidchidx
      qr  = -k_heat*dTdr-rho*D*hidchidr
...
do j=1,nj
   do i=1,ni-1
      ...
      hidchidx=0d0
      hidchidr=0d0
      dchidx=dchidxi*geojacj(1,1,i,j)+dchideta*geojacj(2,1,i,j)
      hidchidx=hidchidx+dchidx*chi
      dchidr=dchidxi*geojacj(1,2,i,j)+dchideta*geojacj(2,2,i,j)
      hidchidr=hidchidr+dchidr*chi
