...
implicit none
   ...
   close(66)
   open(66,file='../data/Turb_energy_'//trim(tmpstring)//'.d')
   read(66,*) ((k_mat(i,j),i=1,ni),j=1,nj)
   close(66)
   open(66,file='../data/dissipation_'//trim(tmpstring)//'.d')
   read(66,*) ((d_mat(i,j),i=1,ni),j=1,nj)
   close(66)
   ...
   E_mat(:,:)=p_mat(:,:)/(gamma-1d0)+0.5d0*rho_mat(:,:)*(u_mat(:,:)**2+v_mat(:,:)**2)
   H_mat(:,:)=E_mat(:,:)/rho_mat(:,:)+R_gas*T_mat(:,:)
   ...
   call vtk_scalar(E_mat,"Energy")
   call vtk_scalar(H_mat,"Enthalpy")
   call vtk_scalar(T_mat,"Temperture")
   call vtk_scalar(k_mat,"Turb_energy")
   call vtk_scalar(d_mat,"dissipation")
